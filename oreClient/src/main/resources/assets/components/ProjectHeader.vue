            ...
            <div class="col-md-6">
                <div class="project-header">
                    <div class="project-path">
                        <a :href="routes.Users.showProjects(p.namespace.owner).absoluteURL()">{{ p.namespace.owner }}</a>
                        /
                        <a class="project-name" :href="routes.project.Projects.show(p.namespace.owner, p.namespace.slug).absoluteURL()">{{ p.name }}</a>
                    </div>
                    <div>
                        <i class="minor" :title="p.summary"> {{p.summary}} </i>
                    </div>
                </div>
            </div>
            <div class="col-md-6">
                    ...
                    <template v-if="permissions.includes('mod_notes_and_flags') || permissions.includes('view_logs')">
                        ...
                        <ul class="dropdown-menu" aria-labelledby="admin-actions">
                            <li v-if="permissions.includes('mod_notes_and_flags')">
                                <a :href="routes.project.Projects.showFlags(p.namespace.owner, p.namespace.slug).absoluteURL()">Flag history
                                    (@p.flagCount) </a>
                            </li>
                            <li v-if="permissions.includes('mod_notes_and_flags')">
                                <a :href="routes.project.Projects.showNotes(p.namespace.owner, p.namespace.slug).absoluteURL()">Staff notes
                                    (@p.noteCount) </a>
                            </li>
                            <li v-if="permissions.includes('view_logs')">
                                <a :href="routes.Application.showLog(null, null, p.plugin_id, null, null, null, null).absoluteURL()">User
                                    Action Logs</a>
                            </li>
                            <li>
                                <a :href="'https://forums.spongepowered.org/users/' + p.namespace.owner">Owner on forum <i
                ...
                <div class="navbar navbar-default project-navbar pull-left">
                    <div class="navbar-inner">
                        <ul class="nav navbar-nav">
                            <!-- Tabs -->
                            <li id="home" :class="tabActive('home')">
                                <a href="TODO">
                                    <i class="fas fa-plug"></i> @messages("project.home")</a>
                            </li>

                            <li id="versions" :class="tabActive('versions')">
                                <a href="TODO">
                                    <i class="fas fa-download"></i> @messages("project.versions")
                                </a>
                            </li>

                            <li id="docs" :class="tabActive('docs')">
                                <a href="TODO">
                                    <i class="fas fa-book"></i> @messages("project.docs")</a>
                            </li>

                            <!--
                            <li v-if="p.discourse.topic_id" id="discussion" :class="tabActive('discussion')">
                            ...
                            <li v-if="p.settings.homepage" id="homepage">
                                <a :title="p.settings.homepage" target="_blank" rel="noopener"
                                   :href="routes.Application.linkOut(p.settings.homepage).absoluteURL()">
                                    <i class="fas fa-home"></i> Homepage <i class="fas fa-external-link-alt"></i></a>
                            </li>

                            <li v-if="p.settings.issues" id="issues">
                                <a :title="p.settings.issues" target="_blank" rel="noopener"
                                   :href="routes.Application.linkOut(p.settings.issues).absoluteURL()">
                                    <i class="fas fa-bug"></i> Issues <i class="fas fa-external-link-alt"></i></a>
                            </li>

                            <li v-if="p.settings.sources" id="source">
                                <a :title="p.settings.sources" target="_blank" rel="noopener"
                                   :href="routes.Application.linkOut(p.settings.sources).absoluteURL()">
                                    <i class="fas fa-code"></i> Source <i class="fas fa-external-link-alt"></i>
                                </a>
                            </li>

                            <li v-if="p.settings.support" id="support">
                                <a :title="p.settings.support" target="_blank" rel="noopener"
                                   :href="routes.Application.linkOut(p.settings.support).absoluteURL()">
    ...
    export default {
        ...
        computed: {
            ...
            staredClasses: function () {
                return {
                    fas: this.p.user_actions.stared,
                    far: !this.p.user_actions.stared,
                    'fa-star': true
                }
            },
            isOwner: function () {
