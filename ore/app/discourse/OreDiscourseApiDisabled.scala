...
class OreDiscourseApiDisabled[F[_]](implicit F: MonadError[F, Throwable]) extends OreDiscourseApi[F] {
  ...
  override def postDiscussionReply(project: Project, user: User, content: String): F[DiscoursePost] =
    F.raiseError(new Exception("Tried to post discussion with API disabled"))
